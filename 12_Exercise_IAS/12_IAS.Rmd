...
```{r IAS}
IAS <- function(N = 100, omega = 0.01, intra = 10, inter = 3, delta = 0.1, convergence = 0.001)
{
...
Simulate the case giving a "no opinion" state (delta=0.2, intra=14, inter=4). Simulate a 100 times and visualize the distribution of results.

```{r case00}
  # Your code here
```

Simulate the case giving a consensus state (delta=0.2, intra=24, inter=8). Simulate a 100 times and visualize the distribution of results.
...
Simulate the case in which polarization is possible (delta=0.2, intra=44, inter=6). Simulate a 100 times and visualize the distribution of results. How often do you get a polarized outcome?
